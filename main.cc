...
int main() {    
    ...
    Arbol a;
    for (int i = 0; i < M; ++i) {
        Individuo ind(N);         //Pasar a individuo el numero de par de cromosomas
    ...
    while (funciones != "acabar") {
        if (funciones == "anadir_individuo") {
            ...
            ind.leer_comp_gen_y_cromosomas_sex(lx, ly);
            for (int i = 1; i <= N; ++i) {
                ind.leer_cromosomas_nor(i, l[i]);      //posicion, numero de genes 
            }
            ...
            if (p.existe(padre) and p.existe(madre) and not p.existe(hijo)) {
                ...
                else cout << "  no es posible reproduccion sexual" << endl;
            }
            else cout << "  error" << endl;
        }
